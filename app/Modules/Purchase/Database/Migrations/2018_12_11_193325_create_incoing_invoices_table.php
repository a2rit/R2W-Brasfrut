...
class CreateIncoingInvoicesTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('incoing_invoices', function (Blueprint $table) {
            $table->increments('id');
            $table->string('idUser',30);
            $table->string('codSAP',30)->nullable();
            $table->string('idPurchaseOrder',30)->nullable();
            ...
            $table->boolean('is_locked')->defaut(false);
            $table->boolean('dbUpdate')->default(false);
            $table->timestamps();
        });
    }
    ...
    {
        Schema::dropIfExists('incoing_invoices');
    }
}
